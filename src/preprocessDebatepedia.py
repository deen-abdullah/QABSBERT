#
# The purpose of this program is to generate dataset for query focused abstractive summary from Debatepedia dataset
# Input: Dataset from Debatepedia
...
def formatToLines(targetFiles):
	...
	for corpus_type in targetFiles:
		...
		for fileNumber in range (1, numberOfFiles):
			...
			dataset.append(d)
			if (len(dataset) > 2000):
				pt_file = "{:s}.{:s}.{:d}.json".format("../json_data/debatepedia", corpus_type, p_ct)
...
# export CLASSPATH=/home/shifa/stanford-corenlp-full-2018-10-05/stanford-corenlp-3.9.2.jar
# Purpose: to tokenize the Debatepedia dataset using StanfordCoreNLP
...
# Purpose: to make the debatepedia dataset in Binary form
# Parameter:  Input -- 
#			  Output --
# Return:
...
# Purpose: to write summary and document in coresponding directory
# Parameter:  Input --
#			  Output --
# Return:
def writeFile (txt, entryNo, targetFile, type):
...
# Purpose: to generate query focused abstractive summary from Debatepedia dataset
# Parameter:  Input -- 
#				    -- 
#			  Output --
# Return:
def main () :
	...
	for targetFile in targetFiles:
		print(targetFile)
		myDir = "../raw_stories/" + targetFile + "_content"
		...
		for line in myFile:
			count = count + 1
			#print(line[4:-7])
			writeFile(line[4:-7], count, targetFile, 'document')
			
		myFile.close()
		document = count

		print('Summary:')
		myDir = "../raw_stories/" + targetFile + "_summary"

		count = 0
		myFile = open("../raw_stories/test_summary","r")
		for line in myFile:
			count = count + 1
			#print(line[4:-7])
			writeFile(line[4:-7], count, targetFile, 'summary')
			
		myFile.close()
		summary = count
		
		if document == summary:
			print ("Successfully writing for "+targetFile+" is done!!!")

			if targetFile is 'test':
				test = document
			elif targetFile is 'train':
				train = document
			else: 
				valid = document
		else:
			print("Number of file for document and summary mismatched for "+targetFile)
			break
	...
	print ("Total number of Train Files: "+str(train))
	print ("Total number of Test Files: "+str(test))
	print ("Total number of Valid Files: "+str(valid))
	print ("Done ! ! !")
